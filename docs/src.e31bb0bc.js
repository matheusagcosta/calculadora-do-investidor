...
function attFooter() {
  ...
  document.getElementById("vPM").innerHTML = "<span class=\"valuesFoot\" id=\"vPM\">R$ ".concat(avgPrice, "</span>");
}
...
function wipeOut(z) {
  ...
  for (u = z + 1; u < count; u++) {
    if (u == count - 1) {
      document.forms[u].elements[2].innerHTML = "<button id=\"add\" value=\"\" onclick=\"calcPrice(".concat(document.forms.length - 2, ")\"></button>");
    } else {
  ...
  document.forms[count - 1].innerHTML += "<button id=\"add\" value=\"\" onclick=\"calcPrice(".concat(document.forms.length - 2, ")\"></button>"); // remove specified section
